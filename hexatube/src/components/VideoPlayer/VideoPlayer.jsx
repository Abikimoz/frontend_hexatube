...
function VideoPlayer({ id, name, preview, video, type, isAuthenticated, rating }) {
	...
	const handleLikeDislike = async (likeOrDislike) => {
		try {
			await axios.post(`https://hexatube.fun/api/video/${likeOrDislike}`, { id },
				{
					headers: {
	...
	return (
		<div className="videoplayer-box">
			<button onClick={() => handleLikeDislike('like')} disabled={!isAuthenticated}>👍 {likes}</button>
			...
			<button onClick={() => handleLikeDislike('dislike')} disabled={!isAuthenticated}>👎 {dislikes}</button>
		</div>
	);
}
